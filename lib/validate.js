...
validate = (data, options={}) => {
  const type = (guessObjectType(data, true) || "").toLowerCase() || "resource"
...
for (let type of types) {
  ...
  validate[type] = (data, options={}) => {
    const { unknownFields, schemes, rememberSchemes } = options
    const schemeList = schemes || rememberSchemes || []
    ...
    if (type === "scheme" && rememberSchemes && !errors.length && data.uri) {
      const found = schemeList.findIndex(s => s.uri === data.uri)
      if (found >= 0) {
    ...
    if (type === "concept" && Array.isArray(data.inScheme)) {
      data.inScheme.forEach(scheme => {
        const storedScheme = schemeList.find(s => s.uri === scheme.uri) || {}
        const { namespace, notationPattern, uriPattern } = { ...storedScheme, ...scheme }

        const { uri, notation } = data
